...
```{r}
ggplot(edibnb, aes(x = price)) + geom_histogram(binwidth = 50) + facet_wrap(~ neighbourhood, ncol = 3)
```
...
ggplot(data = Top5_neighbourhood, 
       mapping = aes(x = neighbourhood, fill = price, color = price)) +
...
ggplot(edibnb, aes(x = review_scores_rating)) +
  ...
  labs(title = "What neighbourhoods get the highest ratings?",
       x = "Review rating score",
       y = "Amount of reviews")

```
This scatter plot shows what scores each neigbourhood gets, and how many reviews leave this score.
